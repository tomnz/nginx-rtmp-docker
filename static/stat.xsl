<xsl:stylesheet version="1.0" 
  ...
  <xsl:template match="/">
    <html>
      ...
      <body>
        ...
        built 
        <xsl:value-of select="/rtmp/built"/>
  ...
  <xsl:template name="showsize">
    ...
    <xsl:choose>
      <xsl:when test="$sizen >= 1073741824">
        <xsl:value-of select="format-number($sizen div 1073741824,'#.###')"/> T
      </xsl:when>
      <xsl:when test="$sizen >= 1048576">
        <xsl:value-of select="format-number($sizen div 1048576,'#.###')"/> G
      </xsl:when>
      <xsl:when test="$sizen >= 1024">
        <xsl:value-of select="format-number($sizen div 1024,'#.##')"/> M
      </xsl:when>
      <xsl:when test="$sizen >= 0">
        <xsl:value-of select="$sizen"/> K
      </xsl:when>
    </xsl:choose>
  ...
  <xsl:template match="publishing">publishing</xsl:template>
  <xsl:template match="active">active</xsl:template>
